...
SANDBOX_BIN=$(nix-build --no-out-link "$SCRIPT_DIR/../fixtures/basic-sandbox.nix")/bin/sandboxed-bash
NET_BIN=$(nix-build --no-out-link "$SCRIPT_DIR/../fixtures/network-allowed.nix")/bin/sandboxed-bash-net
...
SB_HOME="$CAP_OUT"
expect_ok "ran under an ephemeral HOME" \
	"printf '%s' '$SB_HOME' | grep -q '^/private/tmp/sandbox-home\.'"
expect_ok "ephemeral HOME removed after exit" "test ! -e '$SB_HOME'"
...
expect_ok "ephemeral HOME removed after non-zero exit" "test ! -e '$SB_HOME_FAIL'"
...
SB_HOME_NET="$CAP_OUT"
expect_ok "networked ephemeral HOME removed after exit" "test ! -e '$SB_HOME_NET'"
...
expect_ok "proxy process killed on exit" "[ '$(proxy_pids)' = '$PROXY_BEFORE' ]"
...
expect_ok "no per-run temp state left in /tmp" "[ -z '$LEAKED' ]"
